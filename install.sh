...
for key in AI_HF_MODEL AI_MODEL AI_CONTEXT_LENGTH AI_SKIP_PULL AI_AGENT_TOOLSETS AI_APPROVAL_MODE AI_SYNC_REMOTE_USER AI_SYNC_REMOTE_HOST AI_SYNC_REMOTE_PORT AI_SYNC_REMOTE_ROOT AI_SYNC_SSH_KEY AI_SYNC_LOCAL_ROOT AI_SYNC_AI_HOME AI_SYNC_DELETE AI_AUTO_SYNC_FROM_SERVER AI_REQUIRE_SYNC_CONFIG; do
  if [[ -n "${!key:-}" ]]; then
    env_args+=("$key=${!key}")
  fi
done
