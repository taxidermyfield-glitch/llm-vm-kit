...
for key in AI_MODEL_PRESET AI_BACKEND AI_HF_MODEL AI_MODEL_QUANT AI_MODEL AI_CONTEXT_LENGTH AI_AGENT_TOOLSETS AI_APPROVAL_MODE AI_SYSTEM_PROMPT_FILE AI_SYSTEM_PROMPT AI_MEMORY_DIR AI_MEMORY_FILE AI_VLLM_TENSOR_PARALLEL_SIZE AI_VLLM_DTYPE AI_VLLM_GPU_MEMORY_UTILIZATION AI_VLLM_EXTRA_ARGS AI_REQUIRE_SYNC_CONFIG AI_INSTALL_OPTIONAL_BACKENDS; do
  if [[ -n "${!key:-}" ]]; then
    env_args+=("$key=${!key}")
  fi
done
